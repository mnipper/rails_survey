  ...
  def self.to_zip(name, zipped_file, pictures_export_id)
    ...
    Zip::OutputStream.open(zipped_file.path) do |zipfile|
      all.each do |response_image|
        if response_image.picture.exists?
          title = response_image.versioned_question(response_image.response.question_identifier).question_identifier + '-' + 
            response_image.response.id.to_s + '-' + response_image.picture_file_name
